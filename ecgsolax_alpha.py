...
def reset_usb_driver(vendor_id: str = "0403", product_id: str = "6001") -> bool:
    """Reset the USB driver for FTDI devices.
    
    Args:
        vendor_id: USB vendor ID in hex (default: "0403" for FTDI)
        product_id: USB product ID in hex (default: "6001" for FTDI serial)
        
    Returns:
        True if device was reset successfully, False otherwise
    """
...
def create_inverter_device(port: str, slave_address: int, name: str) -> dict:
    """Create and configure a minimalmodbus inverter device.
    
    Args:
        port: Serial port path (e.g., '/dev/ttyUSB0')
        slave_address: Modbus slave address of the inverter
        name: Human-friendly name for the inverter
        
    Returns:
        Dictionary containing device configuration and instrument
    """
...
def set_device(device: dict) -> None:
    """Set the current active inverter device.
    
    Args:
        device: Device dictionary containing device configuration
    """
    try:
...
def read_serial_number(device: dict) -> int:
    """Read the inverter serial number from register 0.
    
    Args:
        device: Device dictionary containing instrument
        
    Returns:
        Serial number read from the device, or 0 if read fails
    """
...
def init_inverters() -> list:
    """Initialize all inverter devices configured in INVERTER_COUNT.
    
    Returns:
        List of initialized device dictionaries
    """
    ...
    for index in range(INVERTER_COUNT):                                                                                     # Create consecutive serial port names
        ...
        time.sleep(5)                                                                                                       # Wait for 5 seconds before adding the device to the result list
...
def read_block(start_addr: int, count: int) -> list | None:
    """Read a contiguous block of registers with retries.
    ...
    Returns:
        List of register values, or None if all retries fail
    """
    ...
    for attempt in range(1, 4):                                                                                                      # Try up to 3 times to read the block of registers
        try:                                                                                                                         # Try to read the block registers  
            ...
            reset_usb_driver()                                                                                                       # Attempt to reset the USB driver
...
def to_signed16(val: int) -> int:
    """Convert a uint16 value to a signed int16 value.
    
    Args:
        val: Unsigned 16-bit integer (0-65535)
...
def get_32bit(regs: list, high_idx: int, low_idx: int) -> int:
    """Combine two uint16 registers into a uint32 value.
    
    Args:
        regs: List of register values
        high_idx: Index of high 16 bits
        low_idx: Index of low 16 bits
...
def publish_metrics(measurement_name: str, metrics_dict: dict) -> None:
    """Publish metrics to MQTT in InfluxDB Line Protocol format.
    
    Args:
        measurement_name: Name of the measurement (e.g., 'inverter_metrics')
...
def collect_program_metrics() -> int:
    """Collect inverter programmed configuration and settings metrics."""
...
def collect_inverter_metrics() -> int:
    """Collect inverter state, voltage, current, and mode metrics."""
    ...
    # blk4 is optional - it's a single register that may fail
...
def collect_mains_metrics() -> int:
    """Collect mains voltage and consumption metrics."""
...
def collect_load_metrics() -> int:
    """Collect inverter output power and load energy metrics."""
...
def collect_bms_metrics() -> int:
    """Collect battery management system metrics."""
...
def collect_pv_metrics() -> int:
    """Collect solar PV panel voltage, current, and energy metrics."""
...
def collect_fan_metrics() -> int:
    """Collect inverter cooling fan metrics."""
...
def collect_temp_metrics() -> int:
    """Collect inverter heatsink and ambient temperature metrics."""
